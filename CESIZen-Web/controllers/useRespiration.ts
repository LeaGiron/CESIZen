...
const EXERCICE_CUSTOM: ExerciceUI = { label: '🎛️ Personnalisé', inspiration: 4, apnee: 0, expiration: 4 };
...
export function useRespiration() {
  ...
  useEffect(() => {
    const charger = async () => {
      try {
        ...
        setExercicesBDD(liste.map((ex: any) => ({
          label: ex.nom_exer,
  ...
  const exercices: ExerciceUI[] = [...exercicesBDD, EXERCICE_CUSTOM];
    ...
    intervalRef.current = setInterval(() => {
      setCompteur((prev) => {
        ...
        if (phase === 'inspiration') {
          ...
          if (cyclesRestants > 1) { setCyclesRestants((c) => c - 1); prochainePhase = 'inspiration'; prochaineDuree = exercice.inspiration; }
          else { setActif(false); prochainePhase = 'repos'; return 0; }
        }
  ...
  return {
    ...
    exercices, indexCustomReel, estCustom, estConnecte,
    dureeCustom, setDureeCustom, nbCycles, setNbCycles,
    demarrer, arreter, colors: getColors(), chargement,
  };
}
