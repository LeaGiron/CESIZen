...
describe('useAdminUtilisateurs - tests de non-régression', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('charge les utilisateurs pour un administrateur', async () => {
    // Le controller vérifie le rôle admin avant de charger la liste des utilisateurs.
  ...
  it('prépare un utilisateur pour modification', () => {
    ...
    expect(result.current.form.mdp_util).toBe('');
  });

  it('annule une modification et remet le formulaire à zéro', () => {
    const { result } = renderHook(() => useAdminUtilisateurs());
