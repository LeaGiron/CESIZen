...
const Connexion = require('@/app/connexion').default;

describe('connexion - tests unitaires', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('affiche les champs du formulaire de connexion', () => {
    // Test unitaire simple : on vérifie seulement le rendu de l’écran.
  ...
  it('appelle la fonction de connexion au clic', () => {
    // Ici on teste uniquement l’action du bouton.
    const { getByText } = render(React.createElement(Connexion));
