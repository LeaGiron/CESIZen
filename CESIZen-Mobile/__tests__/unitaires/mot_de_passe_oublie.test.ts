...
const MotDePasseOublie = require('@/app/mot_de_passe_oublie').default;

describe('mot_de_passe_oublie - tests unitaires', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('affiche le champ email et le bouton d’envoi', () => {
    // Test unitaire du rendu initial.
    const { getByPlaceholderText, getByText } = render(
      React.createElement(MotDePasseOublie)
    );
  ...
  it('appelle la réinitialisation au clic', () => {
    const { getByText } = render(React.createElement(MotDePasseOublie));
